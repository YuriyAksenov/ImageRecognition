...
def forward(parameters, X):
    ...
    hid_1 = sigmoid(np.dot(W1,X)+b1[:, None]);
    hid_2 = sigmoid(np.dot(W2,hid_1)+b2[:, None]);
    outputs = np.dot(W3,hid_2)+b3[:, None];
...
def backward(activations, targets, parameters):
    ...
    output_error = squared_loss(predictions, targets)
    output_delta = deriv_squared_loss(predictions, targets)
    
    # calculate error for hidden_2
    hidden_2_error = np.dot(W3.T, output_delta)
    hidden_2_delta = hidden_2_error * deriv_sigmoid(hid_2)
    
    # calculate error for hidden_1
    hidden_1_error = np.dot(W2.T, hidden_2_delta)
    hidden_1_delta = hidden_1_error * deriv_sigmoid(hid_1)
    
    dW1 = np.dot(hidden_1_delta, X.T)
    db1 = np.sum(hidden_1_delta, axis=1)
    
    dW2 = np.dot(hidden_2_delta, hid_1.T)
    db2 = np.sum(hidden_2_delta, axis=1)
    
    dW3 = np.dot(output_delta, hid_2.T)
    db3 = np.sum(output_delta, axis=1)
